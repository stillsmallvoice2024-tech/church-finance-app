...
│   ├── CategoryLedger.tsx     # Aggregate summary cards (% Allocated, Specific Seeds, Savings Net, Grand Total) + per-category ledger; reads category_opening_balances with old-field fallback
...
`allocationStore` holds all configs (fetched once on first use). `getConfigForDate(configs, date)` returns the most recent locked config whose `start_date` ≤ the target date. Draft configs are never applied.
