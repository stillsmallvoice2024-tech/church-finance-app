...
- **`outflow_type_id` auto-mapping** (batch wizard): after `stage_code_1` is set for a row, `runImport` resolves the outflow type via `getDefaultOutflowTypeForCategory(cat.id, categoryOutflowMaps, outflowTypeOptions)` — looks up the category by name, then gets its first mapped outflow type; falls back to case-insensitive name match if no category found. Non-blocking, no UX change. `useOutflowTypeOptions()` and `useCategoryOutflowTypeMaps()` both called at modal top level.
