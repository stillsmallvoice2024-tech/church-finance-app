...
import { getCurrencyLocale, CURRENCY_SYMBOLS } from './formatters'
import type {
  ReportGroup,
  ReportItem,
  ReportGroupChild,
...
export function exportReportPDF(
  ...
  for (let ti = 0; ti < tables.length; ti++) {
    ...
    for (const group of table.groups) {
      ...
      for (const child of group.children) {
        if (child.kind === 'item') {
          ...
          tableBody.push([child.data.displayLabel, `${sym}${fmt(val, locale)}`])
        } else {
          const sg = child.data
          ...
          for (const item of sg.items) {
            ...
            tableBody.push([`    ${item.displayLabel}`, `${sym}${fmt(val, locale)}`])
          }
          ...
          tableBody.push([
            ...
            { content: `${sym}${fmt(sgTotal, locale)}`, styles: { fontStyle: 'bold', fillColor: [240, 240, 240] as [number, number, number] } },
          ])
        }
      }
      ...
      tableBody.push([
        ...
        { content: `${sym}${fmt(groupTotal, locale)}`, styles: { fontStyle: 'bold' } },
      ])
    ...
    tableBody.push([
      ...
      { content: `${sym}${fmt(tableTotal, locale)}`, styles: { fontStyle: 'bold', fillColor: [30, 58, 138] as [number, number, number], textColor: [255, 255, 255] as [number, number, number] } },
    ])
  ...
  if (tables.filter(t => t.visible).length > 1 && combinedTables.length > 0) {
    ...
    autoTable(doc, {
      ...
      body: [[
        ...
        { content: `${sym}${fmt(combinedTotal, locale)}`, styles: { fontStyle: 'bold', fillColor: [15, 23, 42] as [number, number, number], textColor: [255, 255, 255] as [number, number, number], fontSize: 10, halign: 'right' } },
      ]],
...
export function exportReportExcel(
  ...
  for (let ti = 0; ti < tables.length; ti++) {
    ...
    rows.push([])
    rows.push(['Account / Description', `Amount (${sym})`])
  ...
  if (tables.filter(t => t.visible).length > 1 && combinedTbls.length > 0) {
    ...
    summaryRows.push([])
    summaryRows.push(['Table', `Total (${sym})`])
