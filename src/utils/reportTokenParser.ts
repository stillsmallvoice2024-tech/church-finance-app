...
// 6 capture groups: fn + up to 5 colon-separated segments
const TOKEN_RE =
...
function parseParts(
  ...
  g5: string | undefined,
  g6: string | undefined,
): { category: string; portion: BudgetPortion | undefined; dateFrom: string | undefined; dateTo: string | undefined; dateField: string | undefined } {
  const category = g2?.trim() ?? ''
  const s3 = g3?.trim() || undefined
  const s4 = g4?.trim() || undefined
  const s5 = g5?.trim() || undefined
  const s6 = g6?.trim() || undefined

  const dateField = s6 && DATE_FIELD_KEYS.has(s6) ? s6 : undefined

  // Backward-compat: if g3 is a date string → old {{FN:cat:dateFrom:dateTo}} format
  if (s3 && DATE_RE.test(s3)) {
    return { category, portion: undefined, dateFrom: s3, dateTo: s4, dateField }
  }
  // New format: g3 is a portion key (or absent)
  const portion = (s3 && PORTION_KEYS.has(s3)) ? s3 as BudgetPortion : undefined
  return { category, portion, dateFrom: s4, dateTo: s5, dateField }
}
...
export function parseTokens(text: string): ParsedToken[] {
  ...
  for (const m of text.matchAll(TOKEN_RE)) {
    ...
    const { category, portion, dateFrom, dateTo, dateField } = parseParts(m[2], m[3], m[4], m[5], m[6])
    tokens.push({ raw: m[0], fn, category, portion, dateFrom, dateTo, dateField })
  }
  return tokens
}
...
export function buildTokenString(
  ...
  // Only embed non-default dateField in the key so different columns cache separately
  const fieldSuffix = dateField && dateField !== 'date' ? `:${dateField}` : ''
  if (fn === 'NET') {
    return dateFrom && dateTo ? `{{NET:${dateFrom}:${dateTo}${fieldSuffix}}}` : `{{NET}}`
  }
  const cat         = category.trim()
  const portionPart = portion && portion !== 'all' ? `:${portion}` : ''
  if (dateFrom && dateTo) {
    return `{{${fn}:${cat}${portionPart}:${dateFrom}:${dateTo}${fieldSuffix}}}`
  }
  if (portionPart) {
...
export async function resolveTokens(
  ...
  await Promise.all(
    Array.from(unique.values()).map(async t => {
      ...
      const df = t.dateField || undefined

      let result: QueryResult
      try {
        switch (t.fn) {
          case 'BALANCE':  result = await getCategoryBalance(t.category, dr, t.portion, df);  break
          case 'INFLOWS':  result = await getCategoryInflows(t.category, dr, t.portion, df);  break
          case 'OUTFLOWS': result = await getCategoryOutflows(t.category, dr, t.portion, df); break
          case 'NET':      result = await getNetMovement(dr, df);                              break
...
export function splitByTokens(text: string): Array<string | ParsedToken> {
  ...
  for (const m of text.matchAll(TOKEN_RE)) {
    ...
    const fn = m[1] as TokenFn
    const { category, portion, dateFrom, dateTo, dateField } = parseParts(m[2], m[3], m[4], m[5], m[6])
    segments.push({ raw: m[0], fn, category, portion, dateFrom, dateTo, dateField })
    last = idx + m[0].length
  }
