...
import { ImportModeChooser, getDefaultImportMode, type ImportMode } from '../components/modals/ImportModeChooser'
...
const TABS: { id: Tab; label: string; icon: React.ElementType }[] = [
  { id: 'file',   label: 'Import',       icon: Upload  },
    ...
    <div className="space-y-5">
      ...
      {activeTab === 'file' && (
        <div className="space-y-4">
          ...
          {/* Mode chooser — Guided vs Quick (shell routes to the two flows) */}
          <ImportModeChooser value={importMode} onChange={setImportMode} />

          {/* Guided: launch the step-by-step wizard */}
          {importMode === 'guided' && (
            <div className="rounded-xl border border-gray-200 bg-white p-8 text-center space-y-5">
              ...
              <div>
                <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-100">Guided Import</h2>
              ...
              <p className="text-xs text-gray-400 dark:text-gray-500">
                Excel files only (.xlsx / .xls) · for PDF or CSV, use Quick Import
              </p>
            </div>
          )}

          {/* Quick: existing drag-and-drop flow */}
          {importMode === 'quick' && <>
  ...
  const doSaveInflow = async (pregenRef?: string) => {
    ...
    try {
      ...
      let input: AddInflowInput = {
        ...
        bank_id:                    selectedBank?.id               || undefined,
