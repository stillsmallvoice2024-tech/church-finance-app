...
--   4. Add fx_conversions.org_id index
-- ============================================================


-- ── 1. fx_conversions: add org_id ─────────────────────────────────────────────
-- This table was created before the multi-tenancy migration and was missed.
-- Use nullable first (ON DELETE SET NULL matches other tables) then backfill.

ALTER TABLE public.fx_conversions
  ADD COLUMN IF NOT EXISTS org_id uuid REFERENCES public.organizations(id) ON DELETE SET NULL;

-- Backfill from the related inflow transaction (naira_inflow_id → org_id).
-- Rows without a linked inflow fall back to the primary org (first created).
...
-- ── 2. Enable RLS on fx_conversions ───────────────────────────────────────────
